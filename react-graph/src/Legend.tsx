...
function Legend(props: IProps) {
  ...
  const fontSizeMap = React.useRef<Map<string, number>>(new Map());
  const [smallestFontSize, setSmallestFontSize] = React.useState<number>(1);

  const RegisterFontSize = React.useCallback((requesterID: string, fontsize: number) => {
    fontSizeMap.current.set(requesterID, fontsize);
    const values = [...fontSizeMap.current.values()];
    const newMinVal = values.length > 0 ? Math.min(...values) : 1;
    setSmallestFontSize(newMinVal);
  }, [])

  const UnRegisterFontSize = React.useCallback((requesterID: string) => {
  ...
  const legendContextValue = React.useMemo<ILegendContext>(() => {
    const scrollBarSpace = (hasScroll ? GetScrollbarWidth() : 0);

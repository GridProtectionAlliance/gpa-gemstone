...
function IsCron(value: string): boolean {
    const minuteToken = "[1-5]?[0-9]"; // Do we do this string manipulation every time? A way to save on this memory?
    const hourToken = "[1]?[0-9]|2[0-3]";
    const monthDayToken = "[1-9]|[12][0-9]|3[01]";
    const monthToken = "[1-9]|1[0-2]";
    const weekdayToken = "[0-6]";
    const regexCommonExpression = "((?:\\*)|(?:({token},)+{token}{1})|(?:{token}-{token})|(?:(?:\\*\\/)?{token}))";
    const regexToken = "(^[*]$)|(^" +

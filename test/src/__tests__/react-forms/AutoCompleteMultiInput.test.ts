//******************************************************************************************************
//  AutoCompleteMultiInput.test.ts - Gbtc
//
...
import { AutoCompleteMultiInputRoute } from "../../components/App";
import { AutoCompleteMultiInputEmptyID, AutoCompleteMultiInputPopulatedID } from "../../components/react-forms/AutoCompleteMultiInput";

const rootURL = `http://localhost:${global.PORT}/${AutoCompleteMultiInputRoute}`;
...
beforeEach(async () => {
    ...
    await driver.wait(until.titleIs(AutoCompleteMultiInputRoute), 10000);
});
...
describe('AutoCompleteMultiInput', () => {
    it('shows the plus icon when the array is empty', async () => {
        const container = await driver.wait(until.elementLocated(By.id(AutoCompleteMultiInputEmptyID)), 300);
    ...
    it('shows the plus icon on the last row when the array has data', async () => {
        const container = await driver.wait(until.elementLocated(By.id(AutoCompleteMultiInputPopulatedID)), 300);
        const plusIcons = await container.findElements(By.css('svg.feather.feather-plus-circle'));
        expect(plusIcons.length).toBe(1);
        const plusBtn = await plusIcons[0].findElement(By.xpath('ancestor::button'));
        const display = await plusBtn.getCssValue('display');

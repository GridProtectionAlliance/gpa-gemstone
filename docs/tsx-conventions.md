...
### Comments inside long functions

For a long function with several distinct processing stages, add a short
plain-language comment at each major transition so a reader can scan the
workflow without tracing every statement. Describe the purpose of the stage or
why it is needed; do not restate obvious code.

```ts
// Match uploaded rows to existing records before validating field values.
const recordsByID = new Map(records.map(record => [record.ID, record]));
...

// Block submission when any row fails model-specific validation.
const hasErrors = rows.some(row => row.Errors.length > 0);
```

Do not use comments to compensate for unrelated responsibilities in one
function. If a stage has a clear input and output and can be named cleanly,
prefer extracting a small pure helper.

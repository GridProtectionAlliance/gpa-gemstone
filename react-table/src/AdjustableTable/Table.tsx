...
export default function AdjustableTable<T>(props: React.PropsWithChildren<TableProps<T>>) {
    ...
    React.useEffect(() => { autoWidth.current = new Map<string, IAutoWidth>(); setAutoWidthVersion(0) }, [currentTableWidth])
    ...
    const setWidth = React.useCallback((colKey: string, key: string | number, width: number) => {
        ...
        if (!autoWidth.current.has(colKey))
            autoWidth.current.set(colKey, { maxColWidth: width, width: new Map<string | number, number>([[key, width]]), enabled: true, adjustement: 0 });
        ...
        //cancel ref
        //Add a Timer - runs within 0.1 ms from when the Timer started to avoid React thinking this is an indinfinte loop....
    ...
    const setAdjustment = React.useCallback((colKey: string, w: number) => {
        if (!autoWidth.current.has(colKey))
            autoWidth.current.set(colKey, { maxColWidth: 0, width: new Map<string | number, number>(), enabled: true, adjustement: w });
        else 
            autoWidth.current.get(colKey)!.adjustement = autoWidth.current.get(colKey)?.adjustement ?? 0 + w;
    ...
    const setMinWidth = React.useCallback((colKey: string, w: number) => {
        if (!autoWidth.current.has(colKey))
            autoWidth.current.set(colKey, { maxColWidth: 0, width: new Map<string | number, number>(), enabled: true, adjustement: 0, minWidth: w });
        else if (autoWidth.current.get(colKey)?.minWidth == w)
            return
    ...
    const setMaxWidth = React.useCallback((colKey: string, w: number) => {
        if (!autoWidth.current.has(colKey))
            autoWidth.current.set(colKey, { maxColWidth: 0, width: new Map<string | number, number>(), enabled: true, adjustement: 0, maxWidth: w });
        else if (autoWidth.current.get(colKey)?.maxWidth == w)
            return
    ...
    return (
        <table
            ...
            style={props.TableStyle ?? defaultTableStyle} ref={tblref}
        >
